<?php
namespace Sedmit\Ronda\Components;
...
class AtolPrinter
{
    ...
    private function postData($id, $data) {
        $response = $this->client->post('http://127.0.0.1:16732/requests', [
                                        'json' => [
                                            'uuid' => $id,
                                            'request' => $data]
                                            ]);
    ...
    /*  if (!$this->connected) {
        ...
        $task = array('type' => 'getDeviceStatus');
/*         $response = $this->client->post('http://127.0.0.1:16732/requests', [
                                        'json' => [
                                            'uuid' => $newId,
                                            'request' => array('type' => 'getDeviceStatus')]
                                            ]); */
        //$result = $response->getReasonPhrase();
        $response = $this->postData($newId, $task);
